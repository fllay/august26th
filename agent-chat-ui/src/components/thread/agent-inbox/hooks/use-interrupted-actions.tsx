...
interface UseInterruptedActionsValue {
  ...
  submitDecision: (decision: Decision, successMessage: string) => Promise<void>;
  handleResolve: (
...
export default function useInterruptedActions({
  ...
  const interruptKey = useMemo(() => {
    ...
    const actionId =
      action && "id" in action ? String((action as { id: unknown }).id) : undefined;
    const actionTool =
      action && "tool" in action
        ? String((action as { tool: unknown }).tool)
        : "";
    return (
      interrupt?.id ??
      actionId ??
      `${action?.name ?? "unknown"}:${actionTool}`
    );
  }, [interrupt]);
  const hitlValue = useMemo(
    () => interrupt.value as HITLRequest | undefined,
    [interrupt],
  );
