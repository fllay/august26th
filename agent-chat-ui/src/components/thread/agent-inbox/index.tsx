...
export function ThreadView({ interrupt }: ThreadViewProps) {
  ...
  const isMinimalWebApproval =
    interrupts.length === 1 &&
    activeInterrupt?.value?.action_requests?.length === 1 &&
    activeInterrupt?.value?.action_requests?.[0]?.name === "web_search";
  ...
  return (
    <div
      className={cn(
        ...
        isMinimalWebApproval ? "p-0" : "rounded-2xl bg-gray-50 p-8",
      )}
    >
      {showSidePanel && !isMinimalWebApproval ? (
        <StateView
        ...
        <div className="flex w-full flex-col gap-6">
          {interrupts.length > 1 && !isMinimalWebApproval && (
