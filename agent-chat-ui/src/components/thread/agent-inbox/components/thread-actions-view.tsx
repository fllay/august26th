...
export function ThreadActionsView({
  ...
  const isMinimal = minimal && !hasMultipleActions;
  ...
  if (isMinimal) {
    return (
      <div className="flex w-full max-w-full flex-col items-start gap-4 rounded-lg border border-gray-200 bg-white p-4">
        <p className="text-base font-semibold text-gray-900">{currentTitle}</p>
        <div className="flex w-full items-center gap-2">
          <Button
            className="flex-1 bg-black text-white hover:bg-black/90"
            onClick={() =>
              submitDecision({ type: "approve" }, "Web search approved.")
            }
          ...
          <Button
            ...
            onClick={() =>
              submitDecision(
                { type: "reject", message: "User denied web search." },
                "Web search denied.",
              )
            }
            disabled={actionsDisabled}
