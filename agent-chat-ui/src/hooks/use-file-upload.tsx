...
export function useFileUpload({
  ...
  const handleFileUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (invalidFiles.length > 0) {
      toast.error(
        `You have uploaded invalid file type. Please upload a ${SUPPORTED_FILE_LABEL} file.`,
      );
    }
  ...
  const handlePaste = async (
    ...
    if (invalidFiles.length > 0) {
      toast.error(
        `You have pasted an invalid file type. Please paste a ${SUPPORTED_FILE_LABEL} file.`,
      );
    }
    if (duplicateFiles.length > 0) {
      toast.error(
