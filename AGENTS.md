...
- When a form is being created from a browser-scoped Google OAuth session, prefer the per-user managed Apps Script runtime for native form-to-sheet linking before trying the shared runtime. This avoids cross-user shared-runtime failures during create-and-link flows.
